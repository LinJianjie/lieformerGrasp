...
class SO2Matrix(_base.SOMatrixBase):
    ...
    def inv_left_jacobian(cls, phi):
        ...
        if len(small_angle_inds) > 0:
            jac[small_angle_inds] = torch.eye(cls.dim, dtype=phi.dtype, device=phi.device).expand(
        ...
        if len(large_angle_inds) > 0:
            ...
            A = hacha * \
                torch.eye(cls.dim, dtype=phi.dtype, device=phi.device).unsqueeze_(
    ...
    def left_jacobian(cls, phi):
        ...
        if len(large_angle_inds) > 0:
            ...
            A = (s / angle).unsqueeze_(dim=1).unsqueeze_(
                dim=2).expand_as(jac[large_angle_inds]) * \
                torch.eye(cls.dim, dtype=phi.dtype, device=phi.device).unsqueeze_(dim=0).expand_as(
