...
from pytorch_utils.components.lie_groups.so2 import SO2Matrix
    ...
    def odot(cls, p, directional=False):
        ...
        if p.shape[1] == cls.dim - 1:
            ...
            if not directional:
                result[:, 0:2, 0:2] = torch.eye(
                    cls.RotationType.dim, dtype=p.dtype, device=p.device).unsqueeze_(dim=0).expand(
        ...
        # Got homogeneous coordinates
            ...
            result[:, 0:2, 0:2] = \
                ...
                torch.eye(cls.RotationType.dim, dtype=p.dytpe, device=p.device).unsqueeze_(dim=0).repeat(
                    p.shape[0], 1, 1)
