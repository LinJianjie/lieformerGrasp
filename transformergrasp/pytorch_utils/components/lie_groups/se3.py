...
from pytorch_utils.components.lie_groups.so3 import SO3Matrix
...
class SE3Matrix(_base.SEMatrixBase):
    ...
    def inv_left_jacobian(cls, xi):
        ...
        rho = xi[:, :3].clone()  # translation part
        phi = xi[:, 3:].clone()  # rotation part
        ...
        if len(small_angle_inds) > 0:
            ...
            torch.eye(cls.dof, dtype=xi.dtype, device=xi.device, out=I)
    ...
    def vee(cls, Xi):
        ...
        xi[:, :3] = Xi[:, :3, 3].clone()
        xi[:, 3:] = cls.RotationType.vee(Xi[:, :3, :3]).clone()
    ...
    def wedge(cls, xi):
        ...
        Xi[:, :3, :3] = cls.RotationType.wedge(xi[:, 3:]).clone()
        Xi[:, :3, 3] = xi[:, :3].clone()
