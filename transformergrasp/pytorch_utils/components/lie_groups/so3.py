...
import pytorch_utils.components.lie_groups._base as _base
from pytorch_utils.components.lie_groups import utils
...
class SO3Matrix(_base.SOMatrixBase):
    ...
    def exp(cls, phi):
        ...
        small_angle_inds = small_angle_mask.nonzero(as_tuple=False).squeeze(dim=1)
        ...
        if len(large_angle_inds) > 0:
            ...
            axis = phi[large_angle_inds] / angle.unsqueeze(dim=1).expand(len(angle), cls.dim)
            s = torch.sin(angle).unsqueeze(dim=1).unsqueeze(dim=2).expand_as(mat[large_angle_inds])
            c = torch.cos(angle).unsqueeze(dim=1).unsqueeze(dim=2).expand_as(mat[large_angle_inds])
    ...
    def inv_left_jacobian(cls, phi):
        ...
        if len(large_angle_inds) > 0:
            angle = angle[large_angle_inds]
            axis = phi[large_angle_inds] / angle.unsqueeze(dim=1).expand(len(angle), cls.dof)

            ha = 0.5 * angle  # half angle
            hacha = (ha / torch.tan(ha)).clone()  # half angle * cot(half angle)
            ha = ha.unsqueeze(dim=1).unsqueeze(dim=2).expand_as(jac[large_angle_inds])
            hacha = hacha.unsqueeze(dim=1).unsqueeze(dim=2).expand_as(jac[large_angle_inds])

            A = hacha * torch.eye(cls.dof, dtype=phi.dtype, device=phi.device).unsqueeze(dim=0).expand_as(
    ...
    def left_jacobian(cls, phi):
        ...
        small_angle_inds = small_angle_mask.nonzero(as_tuple=False).squeeze_(dim=1)
        if len(small_angle_inds) > 0:
            jac[small_angle_inds] = torch.eye(cls.dof, dtype=phi.dtype).expand_as(jac[small_angle_inds]) + \
                                    0.5 * cls.wedge(phi[small_angle_inds])
        ...
        if len(large_angle_inds) > 0:
            ...
            A = (s / angle).unsqueeze(dim=1).unsqueeze(dim=2).expand_as(jac[large_angle_inds]) * \
                torch.eye(cls.dof, dtype=phi.dtype, device=phi.device).unsqueeze(dim=0).expand_as(
                    jac[large_angle_inds])
            B = (1. - s / angle).unsqueeze_(dim=1).unsqueeze(dim=2).expand_as(jac[large_angle_inds]) * \
                utils.outer(axis, axis)
            C = ((1. - c) / angle).unsqueeze(dim=1).unsqueeze(dim=2).expand_as(jac[large_angle_inds]) * \
    ...
    def log(self):
        ...
        if len(small_angle_inds) > 0:
            phi[small_angle_inds, :] = self.vee(
                mat[small_angle_inds] - torch.eye(self.dim, dtype=mat.dtype, device=mat.device).expand_as(
                    mat[small_angle_inds]))

        # Otherwise...
        large_angle_mask = small_angle_mask.logical_not()
        large_angle_inds = large_angle_mask.nonzero(as_tuple=False).squeeze(dim=1)

        if len(large_angle_inds) > 0:
            angle = angle[large_angle_inds]
            sin_angle = angle.sin()
            phi[large_angle_inds, :] = \
                self.vee((0.5 * angle / sin_angle).unsqueeze(dim=1).unsqueeze(dim=1).expand_as(mat[large_angle_inds]) *
