...
class DataUtils:
    @staticmethod
    def normalizedPC(points, max_r_list=None, center_point=None, keepDim=True):
        assert points.shape[2] == 3
        assert points.dim() == 3
        if center_point is None:
            points, center_points = DataUtils.zeroCenter(points)
        else:
            points, center_points = DataUtils.zeroCenter(points, center_point)

        if max_r_list is None:
            r_list = torch.norm(points, dim=2, p=2, keepdim=True)
            max_r_list = torch.max(r_list, dim=1, keepdim=True)[0] * 2
            if keepDim:
                max_r_list = max_r_list.repeat(1, points.shape[1], points.shape[2])
        points_ = points / max_r_list
        return points_, max_r_list, center_points
        # return (points_)
