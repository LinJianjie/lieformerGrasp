...
try:
    print("use my code")
    from pytorch_utils.components.externs_tools.fps import fps
    from pytorch_utils.components.externs_tools.knn import knn
    from pytorch_utils.components.externs_tools.radiusnn import radius
except Exception as e:
    print("use the torch_geometric")
    from torch_geometric.nn import fps
    from torch_geometric.nn import knn
    from torch_geometric.nn import radius

import copy
from pytorch_utils.components.torch_utils import deprecated
...
def farthest_point_sampling(x, ratio):
    ...
    fps_idx_ = fps(x, batch_fps, ratio)
    fps_size = int(fps_idx_.shape[0] / batch_size)
    fps_center_ = x[fps_idx_].reshape(batch_size, -1, dim)
    return fps_center_, x, fps_idx_
...
class BkNN:  # B,N,D
    def __init__(self, points, maxKnn, includeSelf=False, queryPoints=None):
        ...
        if queryPoints is not None:
            ...
            assign_index = K_NN(self.pointCRange, queryPointsCRange, self.maxKnn, batch, batch_query)
        else:
            assign_index = K_NN(self.pointCRange, self.pointCRange, self.maxKnn, batch, batch)
