...
from components.transformerLinearNet.attention_module import *
from components.transformerLinearNet.transformerEncoder_Decoder import *
from components.transformerLinearNet.transformer_utils import *
...
class NetTEncoder(nn.Module):
    def __init__(self, src_vocab, local_attention_size, N=6, d_model=512, d_ff=1024, num_head=8, dropout=0.1,
        ...
        self.last_layer_size = 256 + 512 + 1024
        self.pn_layers = CPointNetLinear([d_model, 256, 512, 1024])

    def forward(self, src_):
        # print("src_: ", src_.shape)
        B, N, D = src_.shape
        z_hat_encoder = self.encoder(src_)
        z_hat = self.pn_layers(z_hat_encoder)
...
class Transformer_Model(nn.Module):
    def __init__(self, src_vocab, tgt_vocab, local_attention_size, N=6, d_model=512, d_ff=1024, num_head=8, dropout=0.1,
                 max_len=5000):
        ...
        self.Encoder = Encoder(input_linear_transform=nn.Sequential(Embeddings(self.src_vocab, d_model)),
                               ...
                               N=self.num_Encoder_Decoder)
        self.Decoder = Decoder(input_linear_transform=nn.Sequential(Embeddings(self.tgt_vocab, d_model)),
        ...
        self.model = TransformerEncoderDecoder(self.Encoder,
                                               self.Decoder,
                                               transformer_last_layer(d_model, self.tgt_vocab))
