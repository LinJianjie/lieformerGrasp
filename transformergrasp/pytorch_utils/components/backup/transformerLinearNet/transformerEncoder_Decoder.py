...
class Encoder(nn.Module):
    ...
    def __init__(self, input_linear_transform, encode_layer, N):
        super(Encoder, self).__init__()
        self.input_linear_transform = input_linear_transform
        self.layers = clones(encode_layer, N)
        self.norm = nn.LayerNorm(encode_layer.d_model)
        self.duplicated = encode_layer.d_model

    def forward(self, x):
        x = self.input_linear_transform(x)
        x = x.unsqueeze(dim=2)
        x = x.repeat(1, 1, self.duplicated)
        rand_grid = Variable(torch.cuda.FloatTensor(x.size(0), 2, self.duplicated))
        rand_grid.data.normal_(0, 1)
        rand_grid = rand_grid / torch.sqrt(torch.sum(rand_grid ** 2, dim=1, keepdim=True))
        x = torch.cat([x, rand_grid], dim=1)
        for layer in self.layers:
            x = layer(x)
        y = self.norm(x).transpose(2, 1)
        return y
