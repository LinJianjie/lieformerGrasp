...
class NetUtil:
    @staticmethod
    def SeqLinear(channels, batch_norm=True, activation="leakyReLU"):
        if batch_norm:
            if activation == "relu":
                return Seq(*[
                    Seq(torch.nn.Linear(channels[i - 1], channels[i]),
                        torch.nn.LayerNorm(channels[i]),
                        torch.nn.ReLU())
    ...
    @staticmethod
    def SeqPointNetConv1d(channels, batch_norm=True, active="LeakyReLU"):
        if active == "relu":
            return Seq(*[
