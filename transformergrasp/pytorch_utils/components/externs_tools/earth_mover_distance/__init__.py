...
for root, dirs, files in os.walk(os.path.join(get_project_root(), "extensions_build/earch_mover_distance")):
    for file in files:
        if ".so" in file:
            _build = root
            break
# _path = os.path.join(project_root, "extensions_build/earch_mover_distance", _build)
_path = _build
