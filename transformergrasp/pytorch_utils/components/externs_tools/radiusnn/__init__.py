...
for root, dirs, files in os.walk(os.path.join(project_root, "extensions_build/radiusnn")):
    for file in files:
        if ".so" in file:
            _build = root + "/" + file
            break
# _build = "lib/python3.6/site-packages/radiusnn-2.0.0-py3.6-linux-x86_64.egg/radiusnn.cpython-36m-x86_64-linux-gnu.so"
# _path = os.path.join(project_root, "extensions_build/radiusnn", _build)
_path = _build
...
def radius(x: torch.Tensor, y: torch.Tensor, r: float,
    ...
    return torch.ops.my_torch_cluster.radius(x, y, ptr_x, ptr_y, r,
